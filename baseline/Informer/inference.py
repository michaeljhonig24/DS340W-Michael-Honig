...
from data.dataloader import get_data    # 保证FourTupleDataset已经是4元组格式
import os, sys
project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
sys.path.insert(0, project_root)
save_path = os.path.join(project_root, 'checkpoints', 'Informer.pth')
from utils.ucformertool import set_seed, split_and_rename, calculate_r2, calculate_rmse, cal_mess
import argparse
import pandas as pd
...
_, _, test_loader, lab_min, lab_max, lab_mean, lab_std = get_data(data_path, args.batch_size)  # 或只返回test_loader，视你的get_data接口而定
...
datm_data_path = os.path.join(project_root, 'data', 'datm_data.csv')
