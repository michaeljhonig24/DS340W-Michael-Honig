...
class TransformerClimateModel(nn.Module):
    def __init__(self, input_dim, output_dim, model_dim, num_heads, num_encoder_layers, feedforward_dim, dropout_rate):
        ...
        self.dropout = nn.Dropout(dropout_rate)  # 定义dropout模块
    ...
    def forward(self, src):
        ...
        src = self.dropout(src)  # 应用dropout到位置编码后的输出
        output = self.transformer_encoder(src)
        output = self.output_layer(output)  # 不使用平均，直接输出每个时间步的预测
        output = output.permute(1, 0, 2)  # 将维度调整回(batch_size, seq_length, output_dim)
        return output
