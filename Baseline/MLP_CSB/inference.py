...
save_path = os.path.join(os.path.dirname(__file__), '..','..', 'checkpoints', 'mlp_csb.pth')
...
from utils.tools import set_seed, inverse_scale, inverse_standardize, split_and_rename, calculate_r2, calculate_rmse, cal_mess
import time
from dataloader import get_data
from mlp_csb import MLPModel
import argparse
import pandas as pd
...
with torch.no_grad():
    for inputs, targets in tes_loader:
        ...
        # 逆缩放和逆标准化预测和真实标签
        scaled_train_lab_min= torch.tensor(scaled_train_lab_min)
        scaled_train_lab_max = torch.tensor(scaled_train_lab_max)
        train_lab_mean = torch.tensor(train_lab_mean)
        train_lab_std = torch.tensor(train_lab_std)
        ...
        # 计算损失
        for i in range(3):
            y_pred[i].extend(outputs[:, i].cpu().numpy())
            y_true[i].extend(targets[:, i].cpu().numpy())
