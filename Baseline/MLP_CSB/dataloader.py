...
def get_data(parquetpath, bs):
    ...
    train_fea_re = train_fea_tensor.reshape(-1,train_fea_tensor.shape[1])
    train_lab_re = train_lab_tensor.reshape(-1,train_lab_tensor.shape[1])
    va_fea_re = va_fea_tensor.reshape(-1,va_fea_tensor.shape[1])
    va_lab_re = va_lab_tensor.reshape(-1,va_lab_tensor.shape[1])
    tes_fea_re = tes_fea_tensor.reshape(-1,tes_fea_tensor.shape[1])
    tes_lab_re = tes_lab_tensor.reshape(-1,tes_lab_tensor.shape[1])
