...
import time
from src.dataloader_2070 import get_2070data
...
with torch.no_grad():
    for inputs, targets in tes_loader:
        ...
        outputs = torch.cat((tsa, q2m, tw), dim=2)
        # 逆缩放和逆标准化预测和真实标签
...
all_q_r2 = []
all_t_r2 = []
for i, city in enumerate(['London', 'NewYork', 'Rome', 'Shanghai', 'Singapore', 'SaoPaulo']):  #['ld', 'nw', 'rm', 'sh', 'sa', 'sp']
